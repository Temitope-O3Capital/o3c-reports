...
const DEF = {
  header:  () => ({ id: uid(), type: 'header',  logoText: 'O3C Cards', tagline: '',   bg: '#0E2841', textColor: '#ffffff', padding: 32 }),
  text:    () => ({ id: uid(), type: 'text',    html: '<p>Enter your message here...</p>' }),
  image:   () => ({ id: uid(), type: 'image',   src: '', alt: '', link: '', align: 'center', rounded: false }),
  button:  () => ({ id: uid(), type: 'button',  text: 'Click Here', url: '', bg: '#C00000', textColor: '#ffffff', align: 'center', size: 'md', rounded: true }),
...
const TEMPLATES = [
  ...
  },
  {
    ...
    blocks: [
      { ...DEF.header(), bg: '#C00000', tagline: 'Action Required' },
      ...
      { ...DEF.button(), text: 'Pay Now', bg: '#C00000', url: 'https://o3ccards.com/pay' },
      DEF.divider(),
  ...
  },
  {
    ...
    blocks: [
      ...
      { ...DEF.button(), text: 'Activate My Card', bg: '#0E2841' },
      DEF.footer(),
    ],
  },
  {
  ...
  switch (b.type) {
    case 'header':
      return wrap(
        ...
        `<td style="padding:${b.padding || 32}px 40px;background:${b.bg || '#0E2841'};text-align:center;">` +
    ...
    case 'button':
      return wrap(
        ...
        `<a href="${b.url || '#'}" target="_blank" style="display:inline-block;padding:${b.size === 'lg' ? '16px 48px' : '13px 36px'};background:${b.bg || '#C00000'};color:${b.textColor || '#fff'};font-weight:700;font-size:${b.size === 'lg' ? '16px' : '14px'};text-decoration:none;border-radius:${b.rounded !== false ? '6px' : '2px'};letter-spacing:0.02em;">${b.text || 'Click Here'}</a>` +
        `</div>`
      )
...
══════════════════════════════════════════════════════ */
function CanvasBlock({ block, selected, idx, total, onSelect, onUpdate, onMove, onDelete, onDuplicate }) {
  ...
  const renderContent = () => {
    switch (block.type) {
      case 'header':
        return (
          <div style={{ padding: `${block.padding || 32}px 40px`, background: block.bg || '#0E2841', textAlign: 'center' }}>
      ...
      case 'image':
        return (
          <div style={{ textAlign: block.align || 'center', padding: '16px 40px' }}>
            {block.src ? (
      ...
      case 'button':
        return (
          <div style={{ textAlign: block.align || 'center', padding: '8px 40px 20px' }}>
            <div style={{
              display: 'inline-block',
              padding: block.size === 'lg' ? '14px 44px' : '11px 32px',
              background: block.bg || '#C00000',
      ...
      {selected && (
        <div style={{
          ...
          {[
            ...
            <button key={b.title} type="button" title={b.title} disabled={b.disabled} onClick={e => { e.stopPropagation(); b.action() }}
...
/* ══════════════════════════════════════════════════════
   PROPERTIES PANEL
══════════════════════════════════════════════════════ */
function PropsPanel({ block, onUpdate }) {
  if (!block) return (
    <div style={{ padding: 20, textAlign: 'center' }}>
      <span className="material-symbols-rounded" style={{ fontSize: 36, color: 'rgb(var(--fg-4))', display: 'block', marginBottom: 10 }}>touch_app</span>
      <p style={{ fontSize: 12, color: 'rgb(var(--fg-3))' }}>Click a block to edit its properties</p>
    </div>
  )

  const F = ({ label, children }) => (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'rgb(var(--fg-3))', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 5 }}>{label}</label>
      {children}
    </div>
  )
  const Inp = (props) => <input className="form-input" style={{ fontSize: 12, padding: '5px 9px' }} {...props} />
  const ColorRow = ({ label, field, def }) => (
    <F label={label}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <input type="color" value={block[field] || def}
          onChange={e => onUpdate({ [field]: e.target.value })}
          style={{ width: 32, height: 28, borderRadius: 6, cursor: 'pointer', border: '1px solid rgb(var(--border)/0.2)', flexShrink: 0 }} />
        <Inp value={block[field] || def} onChange={e => onUpdate({ [field]: e.target.value })} />
      </div>
    </F>
  )
  const AlignRow = ({ field = 'align', def = 'center' }) => (
    <F label="Alignment">
        ...
        {['left', 'center', 'right'].map(a => (
          <button key={a} type="button" onClick={() => onUpdate({ [field]: a })}
            style={{ flex: 1, padding: '5px 0', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize',
              border: `1.5px solid ${(block[field] || def) === a ? '#3B82F6' : 'rgb(var(--border)/0.2)'}`,
              background: (block[field] || def) === a ? '#EFF6FF' : 'transparent',
              color: (block[field] || def) === a ? '#3B82F6' : 'rgb(var(--fg-3))' }}>
            {a}
          </button>
        ))}
      </div>
  ...
  switch (block.type) {
    case 'header':
      return <>
        ...
        <ColorRow label="Background" field="bg" def="#0E2841" />
        ...
        <F label="Vertical Padding">
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 11, color: 'rgb(var(--fg-4))' }}>px</span>
          </div>
        </F>
      </>
    ...
    case 'text':
      return (
        <div style={{ padding: 10, background: 'rgb(var(--bg-muted))', borderRadius: 8, textAlign: 'center' }}>
          <span className="material-symbols-rounded" style={{ fontSize: 24, color: '#3B82F6', display: 'block', marginBottom: 8 }}>edit</span>
          <p style={{ fontSize: 12, color: 'rgb(var(--fg-2))', lineHeight: 1.6 }}>Click the text block to edit it directly in the canvas.<br />Use the toolbar that appears for formatting.</p>
        </div>
      )
    ...
    case 'image':
      return <>
        <F label="Upload Image">
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgb(var(--bg-muted))', borderRadius: 8, border: '1.5px dashed rgb(var(--border)/0.3)', cursor: 'pointer', fontSize: 12, color: 'rgb(var(--fg-2))' }}>
            ...
            <input type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
              ...
              try {
                ...
                const res = await apiFetch('/api/campaigns/upload-image', { method: 'POST', body: fd, isFormData: true })
    ...
    case 'button':
      return <>
        ...
        <ColorRow label="Background" field="bg" def="#C00000" />
        <ColorRow label="Text Color" field="textColor" def="#ffffff" />
        <AlignRow />
        <F label="Size">
          <div style={{ display: 'flex', gap: 4 }}>
            {[['md', 'Normal'], ['lg', 'Large']].map(([k, l]) => (
              <button key={k} type="button" onClick={() => onUpdate({ size: k })}
                style={{ flex: 1, padding: '5px 0', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer',
                  border: `1.5px solid ${(block.size || 'md') === k ? '#3B82F6' : 'rgb(var(--border)/0.2)'}`,
                  background: (block.size || 'md') === k ? '#EFF6FF' : 'transparent',
                  color: (block.size || 'md') === k ? '#3B82F6' : 'rgb(var(--fg-3))' }}>
                {l}
              </button>
            ))}
          </div>
    ...
    case 'two_col':
      return <>
        <F label="Column Split">
          {[['50/50', 'Equal (50/50)'], ['60/40', '60 / 40'], ['40/60', '40 / 60'], ['70/30', '70 / 30']].map(([v, l]) => (
            <button key={v} type="button" onClick={() => onUpdate({ split: v })}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: '6px 10px', marginBottom: 4, borderRadius: 6, fontSize: 12, cursor: 'pointer',
                border: `1.5px solid ${block.split === v ? '#3B82F6' : 'rgb(var(--border)/0.2)'}`,
                background: block.split === v ? '#EFF6FF' : 'transparent',
                color: block.split === v ? '#3B82F6' : 'rgb(var(--fg-2))' }}>
              {l}
            </button>
          ))}
        </F>
        <F label="Left Column HTML">
          <textarea className="form-input" rows={4} style={{ fontSize: 11, fontFamily: 'monospace' }}
            value={block.leftHtml || ''} onChange={e => onUpdate({ leftHtml: e.target.value })} />
        </F>
        <F label="Right Column HTML">
          <textarea className="form-input" rows={4} style={{ fontSize: 11, fontFamily: 'monospace' }}
            value={block.rightHtml || ''} onChange={e => onUpdate({ rightHtml: e.target.value })} />
        </F>
      </>
      ...
      return <>
        <F label="Footer Text">
          <textarea className="form-input" rows={3} style={{ fontSize: 12 }}
            value={block.text || ''} onChange={e => onUpdate({ text: e.target.value })} />
        </F>
    ...
    default: return <p style={{ fontSize: 12, color: 'rgb(var(--fg-3))' }}>No properties for this block.</p>
  }
}
...
function TemplateGallery({ onPick, onClose, hasBlocks }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div className="card" style={{ width: '100%', maxWidth: 740, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid rgb(var(--border)/0.1)' }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700 }}>Choose a Template</h2>
            <p style={{ fontSize: 12, color: 'rgb(var(--fg-3))', marginTop: 2 }}>Start from a template or build from scratch — you can always switch later</p>
          </div>
          {hasBlocks && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {TEMPLATES.map(tpl => (
              <button key={tpl.id} type="button"
                onClick={() => onPick(tpl)}
                style={{ padding: 16, borderRadius: 12, border: '1.5px solid rgb(var(--border)/0.15)', background: 'rgb(var(--bg-subtle))', cursor: 'pointer', textAlign: 'left' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = tpl.color; e.currentTarget.style.background = tpl.color + '0c' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgb(var(--border)/0.15)'; e.currentTarget.style.background = 'rgb(var(--bg-subtle))' }}>
                ...
                <p style={{ fontSize: 13, fontWeight: 700, color: 'rgb(var(--fg-1))', marginBottom: 4 }}>{tpl.name}</p>
                <p style={{ fontSize: 11, color: 'rgb(var(--fg-3))', lineHeight: 1.5 }}>{tpl.desc}</p>
              </button>
            ))}
          </div>
        </div>
...
export default function EmailBlockEditor({ value, onChange }) {
  const [blocks, setBlocks]       = useState(() => (value?.blocks || []).map(b => ({ ...b, id: b.id || uid() })))
  const [settings]                = useState(value?.settings || { background: '#f4f4f4', contentWidth: 680 })
  ...
  const updateBlock = (id, patch) => push(blocks.map(b => b.id === id ? { ...b, ...patch } : b))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 560, border: '1px solid rgb(var(--border)/0.15)', borderRadius: 10, overflow: 'hidden' }}>
      {/* Toolbar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: 'rgb(var(--bg-surface))', borderBottom: '1px solid rgb(var(--border)/0.1)', flexShrink: 0 }}>
        <button type="button" className="btn btn-ghost btn-sm" onClick={() => setShowTemplates(true)}>
          <span className="material-symbols-rounded" style={{ fontSize: 15 }}>grid_view</span>
          Templates
        </button>
        <span style={{ fontSize: 11, color: 'rgb(var(--fg-4))' }}>·</span>
        <span style={{ fontSize: 11, color: 'rgb(var(--fg-3))' }}>{blocks.length} block{blocks.length !== 1 ? 's' : ''}</span>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
        ...
        {/* LEFT — Block palette */}
        <div style={{ width: 88, flexShrink: 0, borderRight: '1px solid rgb(var(--border)/0.1)', background: 'rgb(var(--bg-subtle))', overflowY: 'auto', padding: '10px 6px' }}>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgb(var(--fg-4))', textAlign: 'center', marginBottom: 10 }}>Blocks</p>
          {PALETTE.map(p => (
            <button key={p.type} type="button"
              onClick={() => addBlock(p.type, selectedId)}
              title={`Add ${p.label} ${selectedId ? 'after selected' : 'at end'}`}
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, width: '100%', padding: '7px 2px', borderRadius: 8, border: '1px solid transparent', background: 'none', cursor: 'pointer', marginBottom: 2 }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgb(var(--bg-surface))'; e.currentTarget.style.borderColor = 'rgb(var(--border)/0.2)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.borderColor = 'transparent' }}>
              <span className="material-symbols-rounded" style={{ fontSize: 18, color: 'rgb(var(--fg-2))' }}>{p.icon}</span>
              <span style={{ fontSize: 9, color: 'rgb(var(--fg-3))', textAlign: 'center', lineHeight: 1.2 }}>{p.label}</span>
            </button>
          ))}
        </div>
            ...
            {blocks.length === 0 ? (
              <div style={{ padding: 60, textAlign: 'center', color: '#bbb' }}>
                ...
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                  <button type="button" className="btn btn-primary btn-sm" onClick={() => setShowTemplates(true)}>Choose Template</button>
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => addBlock('header', null)}>Add Header</button>
                </div>
              </div>
            ) : (
              blocks.map((block, i) => (
                <div key={block.id}>
                  ...
                  <div className="add-between" style={{ height: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.15s' }}
        ...
        {/* RIGHT — Properties */}
        <div style={{ width: 210, flexShrink: 0, borderLeft: '1px solid rgb(var(--border)/0.1)', background: 'rgb(var(--bg-surface))', overflowY: 'auto' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid rgb(var(--border)/0.1)' }}>
            <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgb(var(--fg-3))' }}>
      ...
      {preview && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 2000, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', background: '#0E2841', flexShrink: 0 }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>Email Preview</span>
            <button type="button" onClick={() => setPreview(false)}
              style={{ color: '#fff', background: 'none', border: 'none', fontSize: 13, cursor: 'pointer', padding: '4px 12px' }}>
              Close Preview
            </button>
          </div>
