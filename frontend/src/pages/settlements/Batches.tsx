    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8 }}>
      <thead>
        <tr style={{ background: 'var(--th-bg)' }}>
          ...
          <th style={{ ...thBase, textAlign: 'right' }}>Amount NGN</th>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--th-bg)' }}>
                ...
                <th style={{ ...thBase, textAlign: 'right' }}>Total Amount NGN</th>
                <th style={thBase}>Status</th>
            ...
            <tbody>
              {filteredRows.map(row => (
                <React.Fragment key={row.id}>
                  ...
                  >
                    <td style={tdBase}><NameCell name={row.batch_ref} sub={row.generated_by ?? undefined} avatar={false} /></td>
                    ...
                    <td style={tdBase}>
                      <ActionRow actions={[
                        { icon: expandedId === row.id ? 'expand_less' : 'expand_more', label: expandedId === row.id ? 'Collapse' : 'Expand', onClick: () => toggleExpand(row.id) },
                      ]} />
                    </td>
                  </tr>
                  {expandedId === row.id && (
                    <tr key={`${row.id}-exp`}>
                      <td colSpan={6} style={{ padding: '0 24px 16px', background: 'var(--th-bg)' }}>
