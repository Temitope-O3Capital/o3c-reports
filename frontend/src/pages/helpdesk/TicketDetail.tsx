...
import { RED, GREEN, AMBER, BLUE, NAVY, PURPLE } from '../../lib/design'
    ...
    <div style={{ display: 'flex', flexDirection: isAgent ? 'row-reverse' : 'row', gap: 8, marginBottom: 14 }}>
      ...
      <div style={{ maxWidth: '70%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexDirection: isAgent ? 'row-reverse' : 'row', marginBottom: 3 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--txt)' }}>{sender}</span>
        ...
        <div style={{
          ...
          fontSize: 13.5, color: 'var(--txt)', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
        }}>
          {msg.body_text}
        </div>
      </div>
...
export default function TicketDetail() {
  ...
  const ModalFooter = ({ onConfirm, label, disabled, danger }: { onConfirm: () => void; label: string; disabled?: boolean; danger?: boolean }) => (
    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
      ...
      <button onClick={onConfirm} disabled={disabled || actionLoading}
        style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: danger ? RED : NAVY, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: (disabled || actionLoading) ? 0.6 : 1, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 7, fontSize: 13, background: 'var(--input-bg)', color: 'var(--txt)', outline: 'none', boxSizing: 'border-box',
  }
  ...
  return (
    ...
    >
          ...
          <SectionCard padding={false} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--bdr)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--txt2)', fontFamily: 'Inter, monospace' }}>
                  #{ticket.ticket_ref || ticket.id}
                </span>
                {ticket.ticket_type && (
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: 'var(--chip-bg)', color: 'var(--chip-txt)' }}>
                    {ticket.ticket_type}
                  </span>
                )}
                <StatusBadge status={ticket.status} />
                <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: prColor }} title={ticket.priority} />
                <span style={{ fontSize: 12, color: sla.color, fontWeight: 600 }}>{sla.label}</span>
              </div>
              <h2 style={{ margin: '6px 0 0', fontSize: 16, fontWeight: 700, color: 'var(--txt)', lineHeight: 1.3 }}>
                {ticket.subject}
              </h2>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 12, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => setTransferOpen(true)}
                  style={{ padding: '6px 14px', borderRadius: 7, border: '1px solid var(--bdr)', background: 'var(--card)', color: 'var(--txt)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span className="material-symbols-rounded" style={{ fontSize: 14 }}>swap_horiz</span>
                  Transfer
                </button>
                <button onClick={() => setEscalateOpen(true)}
                  style={{ padding: '6px 14px', borderRadius: 7, border: `1px solid ${AMBER}50`, background: `${AMBER}10`, color: AMBER, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <button onClick={handleClaimTicket} disabled={actionLoading}
                  style={{ padding: '6px 14px', borderRadius: 7, border: `1px solid ${NAVY}30`, background: `${NAVY}08`, color: NAVY, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span className="material-symbols-rounded" style={{ fontSize: 14 }}>person_add</span>
                  Assign to Me
                </button>
                <button onClick={() => setMergeOpen(true)}
                  style={{ padding: '6px 14px', borderRadius: 7, border: '1px solid var(--bdr)', background: 'var(--card)', color: 'var(--txt)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span className="material-symbols-rounded" style={{ fontSize: 14 }}>merge</span>
                  Merge
                </button>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '16px 18px' }}>
              {messages.length === 0 ? (
                <p style={{ color: 'var(--txt2)', fontSize: 13, textAlign: 'center', marginTop: 40 }}>No messages yet.</p>
              ) : (
            ...
            <div style={{ borderTop: '1px solid var(--bdr)', padding: '14px 18px', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12.5, color: 'var(--txt2)', cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                <button onClick={sendReply} disabled={!replyText.trim() || sending}
                  style={{
                    padding: '8px 20px', borderRadius: 8, border: 'none', background: NAVY, color: '#fff',
                    fontSize: 13, fontWeight: 600, cursor: (!replyText.trim() || sending) ? 'not-allowed' : 'pointer',
                    opacity: (!replyText.trim() || sending) ? 0.6 : 1, display: 'flex', alignItems: 'center', gap: 8,
                  }}>
              ...
              {!ctxLoading && tab === 'customer' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {!ticket.customer_name && !ticket.customer_phone && (
                    <p style={{ color: 'var(--txt2)', fontSize: 13, margin: 0 }}>No customer information available.</p>
                  )}
                </div>
              )}
              {!ctxLoading && tab === 'loans' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {(!ctx?.loans || ctx.loans.length === 0) ? (
                    <p style={{ color: 'var(--txt2)', fontSize: 13, margin: 0 }}>
                      {ticket.customer_cif ? 'No active loans for this customer.' : 'No CIF — loan data unavailable.'}
                    </p>
                  ) : ctx.loans.map((l, i) => (
                    <div key={i} style={{ background: 'var(--th-bg)', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: 12.5, fontWeight: 700, color: NAVY, fontFamily: 'Inter, monospace' }}>{l.loan_ref ?? 'Loan'}</span>
                        <span style={{ fontSize: 11, fontWeight: 600, padding: '1px 7px', borderRadius: 10, background: `${BLUE}15`, color: BLUE }}>{l.status}</span>
                      </div>
                      <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                        <div>
                          <div style={{ color: 'var(--txt2)' }}>Outstanding</div>
                          <div style={{ fontWeight: 700, color: 'var(--txt)' }}>{fmtKobo(l.total_outstanding_kobo ?? 0)}</div>
                        </div>
                        {(l.dpd ?? 0) > 0 && (
                          <div>
                            <div style={{ color: 'var(--txt2)' }}>DPD</div>
                            <div style={{ fontWeight: 700, color: (l.dpd ?? 0) > 30 ? RED : AMBER }}>{l.dpd} days</div>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {!ctxLoading && tab === 'fd' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {(!ctx?.fixed_deposits || ctx.fixed_deposits.length === 0) ? (
                    ...
                    <div key={i} style={{ background: 'var(--th-bg)', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--txt)' }}>{fmtKobo(fd.principal_kobo ?? 0)}</span>
                        <span style={{ fontSize: 11, fontWeight: 600, padding: '1px 7px', borderRadius: 10, background: `${GREEN}15`, color: GREEN }}>{fd.status}</span>
                      </div>
                      <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                        <div><div style={{ color: 'var(--txt2)' }}>Rate</div><div style={{ fontWeight: 600 }}>{fd.interest_rate}%</div></div>
                        <div><div style={{ color: 'var(--txt2)' }}>Tenor</div><div style={{ fontWeight: 600 }}>{fd.tenor_days}d</div></div>
                        {fd.maturity_date && <div><div style={{ color: 'var(--txt2)' }}>Matures</div><div style={{ fontWeight: 600 }}>{fd.maturity_date.slice(0, 10)}</div></div>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {(!ctx?.recent_transactions || ctx.recent_transactions.length === 0) ? (
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: i < ctx.recent_transactions!.length - 1 ? '1px solid var(--bdr)' : 'none' }}>
                      <div>
                        <div style={{ fontSize: 12.5, color: 'var(--txt)', fontWeight: 500 }}>{t.description ?? t.transaction_type}</div>
                        <div style={{ fontSize: 11, color: 'var(--txt3)' }}>{t.transaction_date ? t.transaction_date.slice(0, 10) : ''}</div>
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: (t.transaction_type ?? '').toLowerCase().includes('debit') ? RED : GREEN }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(!ctx?.collections_history || ctx.collections_history.length === 0) ? (
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: i < ctx.collections_history!.length - 1 ? '1px solid var(--bdr)' : 'none' }}>
                      <div>
                        <div style={{ fontSize: 12.5, color: 'var(--txt)', fontWeight: 500 }}>{fmtKobo(p.promise_amount_kobo ?? 0)}</div>
                        <div style={{ fontSize: 11, color: 'var(--txt3)' }}>{p.promise_date ? `Promise date: ${p.promise_date.slice(0, 10)}` : ''}</div>
                      </div>
                      <span style={{
              ...
              {!ctxLoading && tab === 'cards' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(!ctx?.cards || ctx.cards.length === 0) ? (
                    ...
                    <div key={i} style={{ background: 'var(--th-bg)', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--txt)' }}>{c.product_name ?? 'Card'}</span>
                        {c.account_status && (
                          <span style={{
                      ...
                      {c.name_on_card && (
                        <div style={{ fontSize: 12, color: 'var(--txt2)' }}>Name on card: <span style={{ color: 'var(--txt)', fontWeight: 500 }}>{c.name_on_card}</span></div>
                      )}
                      {c.account_manager && (
                        <div style={{ fontSize: 12, color: 'var(--txt2)' }}>AM: <span style={{ color: 'var(--txt)' }}>{c.account_manager}</span></div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {[
                ...
                <button key={a.label} onClick={a.onClick}
                  style={{
                    ...
                    borderRadius: 8, background: `${a.color}06`, color: 'var(--txt)',
                    fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, textAlign: 'left',
                  }}>
          ...
          <SectionCard title="Knowledge Base">
            <div>
              <div style={{ position: 'relative' }}>
                ...
                />
                <span className="material-symbols-rounded" style={{ position: 'absolute', left: 10, top: 9, fontSize: 16, color: 'var(--txt3)' }}>search</span>
                {kbSearching && <span style={{ position: 'absolute', right: 10, top: 11 }}><Spinner size={14} /></span>}
              </div>
              {kbResults.length > 0 && (
                <div style={{ marginTop: 8, border: '1px solid var(--bdr)', borderRadius: 8, overflow: 'hidden' }}>
                  {kbResults.map((a, i) => (
                    <div key={a.id} style={{
                    ...
                    >
                      ...
                      <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 2 }}>{a.category} · Click to insert into reply</div>
                    </div>
                  ))}
                </div>
              )}
              {kbQuery && !kbSearching && kbResults.length === 0 && (
                <p style={{ fontSize: 12, color: 'var(--txt3)', marginTop: 8 }}>No articles found for "{kbQuery}"</p>
              )}
            </div>
          </SectionCard>
        </div>
      ...
      <Modal open={transferOpen} onClose={() => { setTransferOpen(false); setTransferTarget('') }} title="Transfer Ticket" width={400}
        ...
        <p style={{ fontSize: 13, color: 'var(--txt2)', margin: '0 0 12px' }}>Select the agent to transfer this ticket to.</p>
      ...
      <Modal open={ptpOpen} onClose={() => { setPtpOpen(false); setPtpAmount(''); setPtpDate('') }} title="Log Promise to Pay" width={420}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12.5, fontWeight: 600, color: 'var(--txt2)', marginBottom: 5 }}>Promise Date</label>
        ...
        footer={
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: RED, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, opacity: actionLoading ? 0.6 : 1 }}>
              {actionLoading && <Spinner size={14} color="#fff" />}
              Escalate
            </button>
          </div>
        ...
        footer={
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleMerge} disabled={!mergeTarget || merging}
              style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: NAVY, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, opacity: (!mergeTarget || merging) ? 0.6 : 1 }}>
      ...
      >
        ...
        {mergeResults.length > 0 && (
          <div style={{ border: '1px solid var(--bdr)', borderRadius: 8, overflow: 'hidden', marginBottom: 12 }}>
            {mergeResults.map((t, i) => (
              <div key={t.id}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12.5, fontWeight: 700, color: NAVY, fontFamily: 'Inter, monospace' }}>#{t.ticket_ref}</span>
                  <StatusBadge status={t.status} />
                </div>
                <div style={{ fontSize: 13, color: 'var(--txt)', marginTop: 2 }}>{t.subject}</div>
              </div>
            ))}
          </div>
        )}
        {mergeTarget && (
          <div style={{ padding: '10px 14px', background: `${NAVY}08`, borderRadius: 8, border: `1px solid ${NAVY}20` }}>
