...
type PrefMap = Record<string, Record<string, boolean>> // event_type → channel → enabled
...
export default function NotificationPreferences() {
  ...
  useEffect(() => {
    fetch('/api/user/notification-preferences', { credentials: 'include' })
      ...
      .then((data: PrefRow[]) => {
        ...
        data.forEach(r => {
          map[r.event_type] ??= {}
          map[r.event_type][r.channel] = r.user_enabled
        })
        setPrefs(map)
      })
  }, [])
  ...
  const toggle = (eventType: string, channel: string) => {
    setPrefs(p => ({
      ...p,
      [eventType]: { ...p[eventType], [channel]: !p[eventType]?.[channel] },
    }))
    setSaved(false)
  }
  ...
  const save = async () => {
    setSaving(true)
    const items = Object.entries(prefs).flatMap(([event_type, chans]) =>
  ...
  const labelFor = (eventType: string) =>
    rows.find(r => r.event_type === eventType)?.label ?? eventType

  const descFor = (eventType: string) =>
    rows.find(r => r.event_type === eventType)?.description ?? ''
