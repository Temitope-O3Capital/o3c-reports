...
// ── Telnyx Voice Section ──────────────────────────────────────────────────────

function TelnyxVoiceSection() {
  const [configured, setConfigured] = useState<boolean | null>(null)

  useEffect(() => {
    apiFetch<{ configured: boolean }>('/api/voice/status', { silent: true })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--txt)' }}>Telnyx Voice</div>
          <div style={{ fontSize: 12.5, color: 'var(--txt2)', marginTop: 3 }}>
            {configured === null
              ? 'Checking status…'
              : configured
                ? 'Your SIP credentials are configured. Use the phone icon (bottom-right) to make and receive calls.'
                : 'SIP credentials not set. Contact your IT admin to configure calling for your account.'}
          </div>
        </div>
        <span style={{
          flexShrink: 0, fontSize: 11.5, fontWeight: 700, padding: '3px 10px', borderRadius: 20,
          background: configured ? `${GREEN}18` : 'var(--chip-bg)',
      ...
      <div style={{ fontSize: 12, color: 'var(--txt3)', lineHeight: 1.6 }}>
        Calling is powered by <strong>Telnyx WebRTC</strong>. SIP credentials are managed centrally by the IT Admin via <strong>Admin → Users</strong>. Contact IT if your calling is not working.
      </div>
    </div>
  )
}
  ...
  return (
    <Page title="Settings" subtitle="Manage your security, integrations, and notification preferences">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 680 }}>
        ...
        <SectionCard title="Telnyx Voice" subtitle="Make and receive calls directly from the workspace">
          <TelnyxVoiceSection />
        </SectionCard>
