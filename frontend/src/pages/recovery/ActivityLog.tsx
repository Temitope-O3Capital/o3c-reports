...
export default function RecoveryActivityLog() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    setErr(null)
    const params = new URLSearchParams({ module: 'recovery', page: String(page), size: String(PAGE_SIZE) })
  ...
  const fieldVisits       = data.filter(e => e.action === 'field_visit_logged').length
  const paymentsApproved  = data.filter(e => e.action === 'payment_approved').length
  const writeoffs         = data.filter(e => e.action.includes('writeoff')).length
  ...
  return (
    <Page
      title="Recovery Activity Log"
      subtitle="Complete audit trail of all recovery team actions"
      actions={
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
        <KpiCard label="Total Actions"     value={loading ? '—' : total.toLocaleString('en-NG')} icon="history"      accent={RED}   loading={loading} />
        <KpiCard label="Field Visits"      value={loading ? '—' : fieldVisits}                   icon="directions_car" accent={BLUE}  loading={loading} />
        <KpiCard label="Payments Approved" value={loading ? '—' : paymentsApproved}              icon="check_circle" accent={GREEN} loading={loading} />
        <KpiCard label="Write-offs"        value={loading ? '—' : writeoffs}                     icon="remove_circle" accent={AMBER} loading={loading} />
      </div>
