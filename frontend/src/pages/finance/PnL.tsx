...
const LINE_COLS: TableCol<ProductLine>[] = [
  { key: 'product', label: 'Product', render: r => <span style={{ fontWeight: 500 }}>{r.product}</span> },
  ...
  { key: 'net',     label: 'Net ₦',      align: 'right', render: r => (
    <span style={{ ...NUM, color: r.net >= 0 ? GREEN : RED, fontWeight: 600 }}>{fmtKobo(r.net)}</span>
  )},
]
...
export default function FinancePnL() {
  ...
  return (
    ...
    >
      {/* KPI strip */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 20, opacity: loading ? 0.6 : 1 }}>
          ...
          {lines.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={lines} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="product" tick={{ fontSize: 11, fill: 'var(--txt2)' }} />
                <YAxis tickFormatter={v => fmtKobo(v)} tick={{ fontSize: 10, fill: 'var(--txt2)' }} width={70} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: 'var(--txt2)', fontSize: 13, flexDirection: 'column', gap: 8 }}>
              <span className="material-symbols-rounded" style={{ fontSize: 28, opacity: 0.3 }}>bar_chart</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 8 }}>
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ ...NUM, fontWeight: 700, color }}>{fmtKobo(value)}</span>
              </div>
            ))}
            <div style={{ borderTop: '1px solid var(--bdr)', paddingTop: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: 'var(--txt2)' }}>Product Lines</span>
                <span style={{ ...NUM, fontWeight: 600 }}>{lines.length}</span>
              </div>
            </div>
          </div>
        </SectionCard>
