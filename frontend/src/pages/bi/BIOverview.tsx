...
function RunStatusPill({ status }: { status: string }) {
  const s = status === 'completed'
    ...
    : status === 'running'
      ...
      : { bg: `${RED}15`, color: RED }
  return <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 8, ...s }}>{status}</span>
}
...
export default function BIOverview() {
  ...
  const REPORT_COLS: TableCol<ReportDef>[] = [
    { key: 'name', label: 'Report', render: r => (
      <div>
        ...
        {r.description && <div style={{ fontSize: 12, color: 'var(--txt3)' }}>{r.description}</div>}
      </div>
    )},
    { key: 'module',      label: 'Module',    render: r => <ModulePill module={r.module} /> },
    { key: 'date_range',  label: 'Range',     render: r => <span style={{ fontSize: 12.5 }}>{r.date_range.replace(/_/g, ' ')}</span> },
    { key: 'run_count',   label: 'Runs',      render: r => <span style={{ ...NUM, fontWeight: 700 }}>{r.run_count}</span> },
    ...
    { key: 'is_public', label: '', render: r => r.is_public
      ? <span style={{ fontSize: 11, fontWeight: 600, color: GREEN }}>Public</span>
      : null
    },
    { key: 'id', label: '', render: r => (
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
        <button onClick={() => runReport(r.id)} style={{ ...btnSecondary, padding: '4px 10px', fontSize: 12 }}>
          Run
        </button>
        <button onClick={() => navigate(`/bi/builder/${r.id}`)} style={{ ...btnSecondary, padding: '4px 10px', fontSize: 12 }}>
          Edit
        </button>
  ...
  const RUN_COLS: TableCol<Run>[] = [
    { key: 'report_name', label: 'Report', render: r => <span style={{ fontWeight: 600 }}>{r.report_name}</span> },
    ...
    { key: 'started_at',  label: 'Started', render: r => <span style={{ fontSize: 12, ...NUM }}>{fmtDatetime(r.started_at)}</span> },
    { key: 'run_by_name', label: 'By',     render: r => <span style={{ fontSize: 12.5 }}>{r.run_by_name ?? '—'}</span> },
    { key: 'error_message', label: '', render: r => r.error_message
      ? <span style={{ fontSize: 11, color: RED, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{r.error_message}</span>
      : null
    },
  ]
      ...
      {loading ? (
        ...
        <>
          <SectionCard title="Saved Reports" badge={reports.length}
            actions={<button onClick={() => navigate('/bi/scheduled')} style={{ ...btnSecondary, fontSize: 12.5 }}>Scheduled Reports</button>}>
