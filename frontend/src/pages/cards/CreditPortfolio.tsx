...
const UTIL_COLORS: Record<string, string> = {
  '0–30%': GREEN, '30–50%': BLUE, '50–80%': AMBER, '80–100%': PURPLE, 'Over limit': RED, 'No limit': 'var(--chart-lbl)',
}

const koboAxisFmt = (v: number) => v >= 1_000_000_00 ? `₦${(v / 1_000_000_00).toFixed(0)}m` : v >= 1_000_00 ? `₦${(v / 1_000_00).toFixed(0)}k` : ''
      ...
      {loading && !kpis ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: SP[3], marginBottom: 14 }}>
            ...
            <KpiCard label="Avg Balance" value={fmtKoboExact(kpis?.avg_balance_kobo ?? 0)} icon="bar_chart" accent={PURPLE} />
            <KpiCard label="Purchases" value={fmtKoboExact(kpis?.purchases_kobo ?? 0)} icon="shopping_cart" accent={BLUE} sub="this cycle" />
            <KpiCard label="Cash Advance" value={fmtKoboExact(kpis?.cash_advance_kobo ?? 0)} icon="local_atm" accent={NAVY} sub="this cycle" />
          </div>
            ...
            <SectionCard title="Utilization Distribution" subtitle="Accounts by balance / credit-limit ratio">
              {util.length > 0 ? (
                <EBar<UtilRow>
                  data={util}
                  xKey="bucket"
                  series={[{ key: 'count', name: 'Accounts', color: NAVY, colorFn: (e) => UTIL_COLORS[e.bucket] ?? NAVY }]}
                  height={220}
                  valueFmt={(v) => fmtNum(v)}
                  legend={false}
                />
            ...
            <SectionCard title="Interest Income Trend" subtitle="Per billing cycle">
              {interest.length > 0 ? (
                <EArea<TrendRow>
                  data={interest}
                  xKey="cycle_date"
                  series={[{ key: 'interest_kobo', name: 'Interest', color: GREEN }]}
                  height={220}
                  endLabel
                  endFmt={koboAxisFmt}
                  hideYAxis
          ...
          <SectionCard title="Receivables & Delinquency Trend" subtitle="Outstanding vs overdue per cycle" style={{ marginBottom: 14 }}>
            {recv.length > 0 ? (
              <EArea<TrendRow>
                ...
                xKey="cycle_date"
                series={[
                  { key: 'outstanding_kobo', name: 'Outstanding', color: NAVY },
                  { key: 'overdue_kobo', name: 'Overdue', color: RED },
                ]}
                height={220}
                endLabel
                endFmt={koboAxisFmt}
                  ...
                  {products.map((p, i) => {
                    ...
                    return (
                      <tr key={p.product} style={{ borderBottom: '1px solid var(--bdr)' }}>
                        ...
                        <td style={{ padding: '10px 12px', textAlign: 'right', ...NUM, fontSize: TEXT.sm, fontWeight: FW.bold, color: 'var(--txt)', fontFamily: INTER }}>{fmtKoboExact(p.outstanding_kobo)}</td>
                        ...
                        <td style={{ padding: '10px 12px', textAlign: 'right', ...NUM, fontSize: TEXT.sm, color: 'var(--txt2)', fontFamily: INTER }}>{fmtKoboExact(p.interest_kobo)}</td>
                        <td style={{ padding: '10px 12px', textAlign: 'right', ...NUM, fontSize: TEXT.sm, color: p.overdue_kobo > 0 ? RED : 'var(--txt2)', fontFamily: INTER }}>{fmtKoboExact(p.overdue_kobo)}</td>
                      </tr>
                    )
                  })}
                </tbody>
