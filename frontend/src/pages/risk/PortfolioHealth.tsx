...
function Tip({ active, payload, label, fmt }: {
  ...
  return (
    <div style={{
      ...
      {label && (
        <div style={{ fontSize: 9.5, fontWeight: FW.semibold, color: 'rgba(255,255,255,.4)', fontFamily: INTER, marginBottom: 7, letterSpacing: .5, textTransform: 'uppercase' }}>{label}</div>
      )}
      {payload.map((p, i) => (
...
const EMPLOYER_COLS: TableCol<EmployerRow>[] = [
  ...
  },
  {
    ...
    render: r => <span style={{ ...NUM, fontWeight: FW.semibold }}>{fmtNum(r.staff_loans_count)}</span>,
  },
  {
    key: 'book_kobo', label: 'Book ₦', align: 'right', sortable: true,
    render: r => <span style={{ ...NUM, fontWeight: FW.semibold }}>{fmtKobo(r.book_kobo)}</span>,
  },
  {
    key: 'pct_of_total', label: '% of Book', align: 'right',
    render: r => <span style={{ ...NUM, fontWeight: FW.semibold, color: r.pct_of_total > 10 ? RED : 'var(--txt)' }}>{fmtPct(r.pct_of_total)}</span>,
  },
  {
    key: 'par30_count', label: 'PAR30 Count', align: 'right',
    render: r => (
      <span style={{ ...NUM, fontWeight: FW.semibold, color: r.par30_count > 0 ? AMBER : GREEN }}>
...
function ChartCard({ title, sub, children }: { title: string; sub: string; children: React.ReactNode }) {
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--card-bdr)', borderRadius: RADIUS.xl, boxShadow: 'var(--card-shadow)', padding: '18px 20px' }}>
        ...
        <ChartCard title="PAR Trend — 12 Months" sub="Portfolio at risk by DPD band">
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={parTrend} margin={{ top: 4, right: 8, bottom: 0, left: -18 }}>
              ...
              <XAxis dataKey="month" tick={{ fontSize: TEXT.xs, fill: 'var(--chart-lbl)', fontFamily: INTER }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: TEXT.xs, fill: 'var(--chart-lbl)', fontFamily: INTER }} axisLine={false} tickLine={false} tickFormatter={v => fmtKobo(v)} width={70} />
          ...
          <div style={{ display: 'flex', gap: SP[4], marginTop: 10, justifyContent: 'flex-end' }}>
            {([[AMBER, 'PAR30'], [RED, 'PAR60'], [DARKRED, 'PAR90']] as const).map(([c, l]) => (
              <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: TEXT.xs, color: 'var(--txt2)', fontFamily: INTER }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: SP[5] }}>
            <div style={{ position: 'relative', flexShrink: 0 }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: TEXT['2xl'], fontWeight: FW.extrabold, color: 'var(--txt)', ...NUM, lineHeight: 1 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 9 }}>
              {bandDist.map(d => (
                <div key={d.band} style={{ display: 'flex', alignItems: 'center', gap: SP[2] }}>
                  <div style={{ width: 10, height: 10, borderRadius: RADIUS.xs, background: BAND_DONUT_COLORS[d.band] ?? 'var(--chart-lbl)', flexShrink: 0 }} />
                  <span style={{ flex: 1, fontSize: TEXT.sm, color: 'var(--txt)', fontWeight: FW.medium }}>{d.band}</span>
                  <span style={{ fontSize: TEXT.base, fontWeight: FW.bold, color: 'var(--txt)', ...NUM }}>{fmtPct(d.pct)}</span>
                </div>
              ))}
            </div>
          </div>
