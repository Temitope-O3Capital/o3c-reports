    ...
    <Page
      ...
      actions={
        <div className="flex items-center gap-2">
          <label className="text-[12px] text-[color:var(--txt2)] font-medium">From</label>
          <input type="date" className="px-3 py-1.5 rounded-lg border border-[var(--bdr)] text-[12px] focus:outline-none"
            value={from} onChange={e => setFrom(e.target.value)} />
          <label className="text-[12px] text-[color:var(--txt2)] font-medium">To</label>
          <input type="date" className="px-3 py-1.5 rounded-lg border border-[var(--bdr)] text-[12px] focus:outline-none"
      ...
      <SectionCard title="Targets Board" badge={targets.length}>
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16">
            <span className="material-symbols-rounded text-[36px] text-[color:var(--txt3)] block mb-2">flag</span>
            <p className="text-[13px] text-[color:var(--txt2)]">No targets found for this period</p>
          </div>
        ) : (
            ...
            <table className="w-full text-[13px]">
                ...
                <tr>
                  {['Agent', 'Date', 'Target', 'Collected', 'Progress', 'Contacts', 'Promises', ''].map(h => (
                    <th key={h}
                      ...
                      style={{ background: NAVY, color: 'var(--txt2)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {targets.map((t, i) => {
                  ...
                  return (
                    <tr key={t.id} className="transition-colors hover:bg-[var(--bg)]"
                      style={{ borderTop: i > 0 ? '1px solid rgba(15,23,42,0.05)' : undefined }}>
                      <td className="px-5 py-3 font-semibold text-[color:var(--txt)]">{t.agent_name}</td>
                      <td className="px-5 py-3 text-[color:var(--txt2)] whitespace-nowrap">{fmtDate(t.target_date)}</td>

                      {/* Editable target cell */}
                      <td className="px-5 py-3">
                        {isEditing ? (
                          <div className="flex items-center gap-1">
                            <span className="text-[color:var(--txt2)] text-[12px]">₦</span>
                            <input
                              type="number" min="0" step="0.01"
                              className="w-28 px-2 py-1 rounded border border-[var(--bdr)] text-[12px] font-mono focus:outline-none focus:ring-2 focus:ring-[#0E2841]/20"
                              value={editing[t.id]}
                          ...
                          <button
                            className="font-mono font-semibold text-[color:var(--txt)] hover:underline cursor-pointer"
                      ...
                      <td className="px-5 py-3" style={{ minWidth: 120 }}>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 rounded-full" style={{ background: 'var(--chip-bg)' }}>
                      ...
                      <td className="px-5 py-3 text-center font-mono text-[color:var(--txt)]">{t.contacts_made}</td>
                      <td className="px-5 py-3 text-center font-mono text-[color:var(--txt)]">{t.promises_obtained}</td>
                        ...
                        {isEditing ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => cancelEdit(t.id)}
                              className="px-2 py-1 rounded text-[11px] font-semibold text-[color:var(--txt2)] bg-black/[0.05]">
                              Cancel
                            </button>
                          </div>
                        ) : (
                          <button onClick={() => startEdit(t)}
                            className="text-[color:var(--txt2)] hover:text-[color:var(--txt)]">
