...
function toList(r: any): string {
  ...
  return to.length === 1 ? to[0].email : `${to[0].email} +${to.length - 1}`
}
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-[340px] flex-shrink-0 border-r overflow-y-auto"
        ...
        {loading && <div className="flex items-center justify-center py-16"><Spinner size={24} /></div>}
        ...
        {messages.map(m => (
          <div key={m.id}
            ...
            <div className="flex items-start justify-between gap-2">
              <p className="text-[13px] font-medium text-slate-800 truncate flex-1">{m.subject || '(no subject)'}</p>
              <StatusPill status={m.status} />
            </div>
            <p className="text-[11px] text-slate-400 mt-0.5">To: {toList(m.recipients)}</p>
        ...
        {!selected ? (
          ...
          <div className="max-w-3xl mx-auto px-8 py-8">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-[18px] font-bold text-slate-900">{selected.subject || '(no subject)'}</h1>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 p-4 rounded-xl text-[12px]"
              ...
              <div>
                ...
                <p className="font-medium text-slate-700 mt-0.5">{selected.from_email}</p>
              </div>
              <div>
            ...
            <p className="text-[12px] text-slate-400 italic">
              Message body is not stored — only delivery metadata is tracked.
            </p>
          </div>
        )}
      </div>
