    ...
    <div className="flex h-full">
      ...
      <div className="w-[320px] flex-shrink-0 border-r overflow-y-auto"
        ...
        <div className="px-4 py-3 border-b flex items-center justify-between"
          style={{ borderColor: 'var(--bdr)' }}>
          <h2 className="text-[14px] font-bold text-[color:var(--txt)]">Inbox</h2>
          <span className="text-[11px] text-[color:var(--txt2)]">{messages.length} messages</span>
        </div>
        ...
        {!loading && messages.length === 0 && !err && (
          <div className="flex flex-col items-center py-16 gap-3 text-[color:var(--txt2)]">
            ...
            <p className="text-[11px] text-[color:var(--txt3)] text-center px-6">
          ...
          <div key={m.id}
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <p className={`text-[13px] truncate ${m.is_read ? 'text-[color:var(--txt2)]' : 'font-semibold text-[color:var(--txt)]'}`}>
                  {m.from_name || m.from_email}
                </p>
              </div>
              <span className="text-[11px] text-[color:var(--txt2)] flex-shrink-0">{relTime(m.received_at)}</span>
            </div>
            <p className={`text-[12px] truncate mt-0.5 ${m.is_read ? 'text-[color:var(--txt2)]' : 'text-[color:var(--txt)]'}`}>
              {m.subject}
            </p>
            <p className="text-[11px] text-[color:var(--txt2)] truncate mt-0.5">
      ...
      <div className="flex-1 overflow-y-auto">
        {!selected ? (
          ...
          <div className="max-w-3xl mx-auto px-8 py-8">
            <h1 className="text-[18px] font-bold text-[color:var(--txt)] mb-4">{selected.subject}</h1>
            <div className="flex items-center gap-3 mb-6 pb-4 border-b"
              ...
              <div>
                <p className="text-[13px] font-semibold text-[color:var(--txt)]">
                  {selected.from_name || selected.from_email}
                </p>
                <p className="text-[12px] text-[color:var(--txt2)]">{selected.from_email} · {relTime(selected.received_at)}</p>
              </div>
            </div>
            {selected.body_html ? (
              <div
                className="prose prose-sm max-w-none text-[color:var(--txt)]"
                dangerouslySetInnerHTML={{ __html: sanitizeHtml(selected.body_html) }}
              />
            ) : (
              <pre className="text-[13px] text-[color:var(--txt)] whitespace-pre-wrap font-sans leading-relaxed">
                {selected.body_text}
              </pre>
            )}
            <div className="mt-8 pt-4 border-t flex gap-2" style={{ borderColor: 'var(--bdr)' }}>
              <a
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-[13px] font-medium text-[color:var(--txt2)] border"
                style={{ borderColor: 'var(--bdr)' }}>
                Back
              </button>
            </div>
