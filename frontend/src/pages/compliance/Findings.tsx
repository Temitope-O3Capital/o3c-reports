...
export default function Findings() {
  ...
  const cols: ColDef<Finding>[] = [
    ...
    { key: 'finding_type', label: 'Type', render: r => (
      <span className="text-[12px] text-slate-600">{r.finding_type || '—'}</span>
    )},
    { key: 'severity', label: 'Severity', render: r => {
      const s = SEVERITY_COLORS[r.severity?.toLowerCase()] ?? { bg: 'rgba(14,40,65,0.06)', color: '#475569' }
      return (
    ...
    { key: 'description', label: 'Description', render: r => (
      <span className="text-[12px] text-slate-600 max-w-[240px] block truncate" title={r.description}>{r.description}</span>
    )},
    { key: 'status', label: 'Status', render: r => <StatusBadge status={r.status} /> },
    { key: 'assigned_to', label: 'Assigned To', render: r => (
      <span className="text-[12px] text-slate-500">{r.assigned_to || '—'}</span>
    )},
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {closeId === r.id && (
          <span className="flex items-center gap-1">
            ...
            <button onClick={() => setCloseId(null)} className="text-[11px] px-1" style={{ color: '#64748B' }}>✕</button>
          </span>
        )}
      </div>
    )},
    ...
    <Page dept="Compliance" title="Audit Findings"
          ...
          <div className="card p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold uppercase text-slate-400 block mb-1">Severity</label>
                ...
                <div>
                  <label className="text-[11px] font-semibold uppercase text-slate-400 block mb-1">Due Date</label>
              ...
              <div>
                <label className="text-[11px] font-semibold uppercase text-slate-400 block mb-1">Finding Type</label>
              ...
              <div>
                <label className="text-[11px] font-semibold uppercase text-slate-400 block mb-1">Description</label>
              ...
              <div>
                <label className="text-[11px] font-semibold uppercase text-slate-400 block mb-1">Assigned To</label>
      ...
      <SectionCard title="Compliance Findings" badge={rows.length}>
        ...
        <div className="flex items-center justify-between px-5 py-3"
          ...
          <span className="text-[12px] text-slate-400">Showing {offset + 1}–{offset + rows.length}</span>
          <div className="flex gap-2">
            <button disabled={offset === 0} onClick={() => load(Math.max(0, offset - PAGE_SIZE))}
              ...
              style={{ borderColor: 'rgba(15,23,42,0.15)', color: NAVY }}>Previous</button>
            <button disabled={rows.length < PAGE_SIZE} onClick={() => load(offset + PAGE_SIZE)}
              className="px-3 py-1 rounded border text-[12px] disabled:opacity-40"
              style={{ borderColor: 'rgba(15,23,42,0.15)', color: NAVY }}>Next</button>
          </div>
        </div>
      </SectionCard>
