...
export default function Sars() {
  ...
  const cols: ColDef<SarRow>[] = [
    ...
    { key: 'subject_name', label: 'Subject', render: r => (
      <span className="text-[12px] italic" style={{ color: 'var(--txt2)' }}>{r.subject_name}</span>
    )},
    ...
    { key: 'transaction_date', label: 'Txn Date', render: r => (
      <span className="text-[12px] whitespace-nowrap" style={{ color: 'var(--txt2)' }}>{fmtDate(r.transaction_date)}</span>
    )},
    { key: 'status', label: 'Status', render: r => <StatusBadge status={r.status} /> },
    { key: 'nfiu_ref', label: 'NFIU Ref', render: r => (
      <span className="font-mono text-[11px]" style={{ color: 'var(--txt2)' }}>{r.nfiu_ref || '—'}</span>
    )},
    { key: 'escalate', label: 'Escalate', sortable: false, render: r => {
      const next = STATUS_FLOW[r.status]
      if (!next) return <span className="text-[11px]" style={{ color: 'var(--txt3)' }}>—</span>
      return escalateId === r.id ? (
        <span className="flex items-center gap-1">
          <input value={escalateNotes} onChange={e => setEscalateNotes(e.target.value)}
            ...
            style={{ borderColor: 'var(--input-bdr)', background: 'var(--input-bg)', color: 'var(--txt)', width: 110 }} />
          ...
          <button onClick={() => setEscalateId(null)}
            className="text-[11px] px-1 py-1 rounded" style={{ color: 'var(--txt2)' }}>✕</button>
        </span>
      ) : (
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="card p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-[12px] mb-4 p-3 rounded" style={{ background: 'rgba(192,0,0,0.05)', color: 'var(--txt2)' }}>
              Subject details are stored encrypted. Enter plaintext — it will be sent as-is and stored encrypted on the server.
            </p>
            <div className="space-y-3">
              <div>
                <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>Subject Full Name</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>ID Number</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>Account Number</label>
                  <input value={newForm.account_number} onChange={e => setNewForm(f => ({ ...f, account_number: e.target.value }))}
                    className="w-full px-3 py-2 rounded border text-[13px] outline-none"
                    style={{ borderColor: 'var(--input-bdr)', background: 'var(--input-bg)', color: 'var(--txt)' }} />
                </div>
                <div>
                  <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>Amount (₦)</label>
              ...
              <div>
                <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>Transaction Date</label>
              ...
              <div>
                <label className="text-[11px] font-semibold uppercase block mb-1" style={{ color: 'var(--txt2)' }}>Summary</label>
                <textarea value={newForm.summary} onChange={e => setNewForm(f => ({ ...f, summary: e.target.value }))}
                  ...
                  style={{ borderColor: 'var(--input-bdr)', background: 'var(--input-bg)', color: 'var(--txt)' }} rows={4} />
              </div>
            </div>
