...
func openRecoveryCase(ctx context.Context, db *core.DB, cif, dpd string, outstanding int64, sourceAssignmentID *int64) (string, int64, error) {
	...
	rows, err := db.PGQuery(ctx, `
		...
		-- Both halves of the merge, because neither side was right alone: main had
		-- the correct 'active' status but dropped product_type, the other branch
		-- carried product_type but reopened the 'open' bug the comment above
		-- describes. product_type defaults to 'card', and 63 of 1,615 live cases are
		-- loans, so leaving it to the default silently files every Udara loan case
		-- as a card.
...
func collectionsOpsSendToRecovery(db *core.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err = tx.QueryRowContext(ctx,
			...
			`INSERT INTO recovery_cases
			   -- product_type from the other branch, 'active' from main: see the
			   -- matching INSERT above for why neither side was correct alone.
			   (case_ref, cif_number, account_cif, outstanding_kobo, total_outstanding_kobo, source_assignment_id, dpd_at_handoff, product_type, status, opened_at, created_at, updated_at)
			 VALUES ($1,$2,$2,$3,$3,$4,$5,
