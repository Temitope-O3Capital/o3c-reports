...
// contact centre.
//
// THAT CONDITION HAS NOW BEEN MET, which is why the footer is back. The paragraph
// above said to restore a truthful footer once an inbound STOP was actually wired to
// a suppression; commit f4c0aa7 ("Honour the opt-out we promise") did exactly that —
// optOutKeyword below reads the inbound body and the handler writes the suppression.
// The promise and the machinery that answers it now ship together, and
// TestThePromisedKeywordIsActuallyHonoured fails if they ever drift apart again.
//
// Restored deliberately during the merge to main: main still carried the no-op stub
// from before the inbound half existed, and taking it would have silently dropped the
// outbound half of a regulatory feature while keeping the inbound half — leaving
// customers told nothing about how to opt out of messages the system could finally
// honour.
