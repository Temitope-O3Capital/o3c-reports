...
func losRecordConsent(db *core.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// consent_type arrived empty, which RecordConsent rejects outright — so every
		// capture 422'd and no consent was ever recorded. "NDPA" is the type Phoenix
		// uses throughout, and the one the read-back in phoenix_offers.go looks for,
		// so writing anything else would store a record our own ledger cannot see.
		payload := map[string]any{
			"consent_type": "NDPA",
			"granted":      true,
		}
		...
		// kept on our own trail rather than dropped.
		user := core.UserFromCtx(r.Context())
