...
3. Enable link branding so tracked links use the O3C domain rather than a generic SendGrid domain.
4. Use a real sender such as `no-reply@o3ccards.com` or `support@o3ccards.com`; do not send from free-mail domains.
5. Configure the Event Webhook URL:
   `/api/campaign-webhooks/email-webhook`
6. Enable Signed Event Webhook and store the public key as `SENDGRID_WEBHOOK_PUBLIC_KEY`.
7. Keep `EMAIL_WEBHOOK_SECRET` only as a development fallback.
